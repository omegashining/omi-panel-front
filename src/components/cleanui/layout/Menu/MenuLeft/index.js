...
const MenuLeft = ({
  ...
  const generateMenuItems = () => {
    const generateItem = item => {
      const { key, title, url, icon, disabled, count, action } = item
      if (item.category) {
      ...
      return (
        <Menu.Item key={key} disabled={disabled}>
          <a href="#" className={style.title} onClick={event => action ? action(event, dispatch) : null}>{title}</a>
          {count && <span className="badge badge-success ml-2">{count}</span>}
          {icon && <span className={`${icon} ${style.icon} icon-collapsed-hidden`} />}
        </Menu.Item>
      )
  ...
  return (
    <Layout.Sider
      ...
      className={classNames(`${style.menu}`, {
        ...
        [style.green]: menuColor === 'green',
        [style.omigray]: menuColor === 'omigray',
